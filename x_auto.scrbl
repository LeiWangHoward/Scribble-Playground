#lang scribble/doc
@(require "common.rkt"sc 
ribble/decode scribble/eval scribble/struct scribble/racket
...
@itemize[
 ...
 @item{@bold{Lexical Structure:} The lexical structure is shown with
  ...
  arrows to indicate potential references within macrod 
  efinitions. Potential arrows are drawn in purple and annotated
 ...
 @item{@bold{Require Annotations:} Right-clicking (or
  Control-clicking on Mac OS X) on the argument to@r 
  acket[require] activates a pop-up menu that lets you open the
  ...
  that @racket[require] expression. Additionally, if nov 
  ariables are used from that require expression, it is colored
...
@itemize[
 ...
 @item{Select the @drlang{Use the language declared in the source}o 
  ption (via the @menuitem["Language" "Choose Language..."] menu
  item), and then specify a specific language as part of thep 
  rogram usually by starting the definitions-window content with
  @hash-lang[].}
...
@itemize[
 ...
 @item{A @deftech{launcher executable} uses the latest version of
  your program source file when it starts. It also accesses libraryf 
  iles from your DrRacket installation when it runs. Since a launcher
